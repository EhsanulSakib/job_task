...
// import navlinks
import { navLinks } from '@/sub-components/navbar/navLinks';
...
const Navbar = () => {
  ...
  return (
    <div className='flex justify-between items-center w-full xl:w-3/5 px-[1%] xl:px-[0%] py-3 mx-auto'>
        ...
        <ul className={`${isOpen ? 'flex' : 'hidden'} lg:flex flex-col lg:flex-row gap-4 font-semibold xl:text-lg`}>
          {navLinks.map((link, index) => (
            <li key={index} className="relative">
              {link.url ? (
                <a href={link.url} className='font-semibold cursor-pointer'>
                  {link.title}
                </a>
              ) : (
                <h2
              ...
              {activeMenu === link.title && link.options && (
                <ul className='lg:absolute top-full mt-2 bg-transparent lg:shadow-lg lg:rounded-md lg:py-2 min-w-[200px]'>
